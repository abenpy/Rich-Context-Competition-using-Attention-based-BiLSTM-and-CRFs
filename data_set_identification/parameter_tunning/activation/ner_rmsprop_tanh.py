...
model = TimeDistributed(Dense(50, activation="tanh"))(main_lstm)
...
# set optimizer 
rmsprop = optimizers.RMSprop(lr=0.001, rho=0.9, epsilon=None, decay=1e-2)
# adam = optimizers.SGD(lr=0.001,  decay=1e-5)
# adam = optimizers.Adam(lr=0.001, epsilon=None, decay=1e-5)
...
model.fit_generator(generator=tr_generator,shuffle=False, epochs=3, verbose=0,callbacks=[history]) #,callbacks=callbacks_list
...
pickle.dump(history_save, open('history_rmsprop_tanh_f', 'wb'))
...
#     pred_ind = np.sort(np.array(pred_ind))
    if len(pred_ind) == 0:
        prediction[pub_id]='Can not find mention list'
    else:
        prediction[pub_id]= np.take(word_flatten,pred_ind)
    Y[pub_id] = pred
