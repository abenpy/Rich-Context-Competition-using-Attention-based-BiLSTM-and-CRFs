...
embedding_matrix[0] = np.ones(100,dtype='float32')*1e-5
...
#Set early stop
filepath="ner_rmsprop_att.h5"
checkpoint = ModelCheckpoint(filepath, monitor='val_acc', verbose=0, save_best_only=True, mode='auto',save_weights_only=True) #val_acc
early_stop = EarlyStopping(monitor='val_loss', patience=5, mode='min') 
tensorboard = TensorBoard(log_dir='./Graph_V3', batch_size = BATCH_SIZE, histogram_freq=2, write_graph=True, write_images=True)
history = NBatchLogger()
callbacks_list = [checkpoint, early_stop, tensorboard，history] 
...
pickle.dump(history_save, open('history_att_selftry', 'wb'))
